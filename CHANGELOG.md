...
**In short:** v2.2.56 found regional banks, healthcare, and consumer discretionary all fail the go-live bar on their own data under the shared 40/20/15/15/10 category weighting, even though the pooled number looked fine. This builds the fix: let each sector's own historical outcomes determine how much Technical/Sentiment/News should matter for that sector, instead of guessing. Two real findings came out of actually building it, not just proposing it. First, the backtest's News sub-score turned out to be structurally useless as a calibration input — it's constant (neutral fallback) across nearly the entire 13.5-year replay, since Alpha Vantage's article cache only covers Q4 2025 onward, so a feature with zero variance was aborting every fit attempt outright; fixed by dropping an uninformative feature from the regression instead of giving up on the whole fit. Second, building a proper safeguard against small per-sector samples (semiconductors: 127 qualifying trades, consumer discretionary: 506, regional banks: 90, healthcare: 68) surfaced a real bug in the weight-clamping math itself — clamping each weight to its bound and then rescaling to sum to 1.0 can push a value back *outside* the bound it just enforced, a bug that's been latent in the original global calibration since it shipped in v2.2.42, just never triggered by real data. With both fixed: semiconductors' own fit was correctly rejected by holdout validation (it would have made things worse — semiconductors already has the weights it was originally tuned around), and consumer discretionary's fit passed and is now live for that sector. Banks and healthcare don't have enough historical trades to trust an independent fit yet and stay on the shared default.
...
**In short:** Asked to review every section of the model for ways to improve it overall, even if that meant a design change — this is that pass. Five real, independent problems found and fixed. The headline: the seasonality calendar (favoring Q4, penalizing Jan/Feb) has been scoring backwards this whole time — flagged as a possible sign error back in v2.2.42 but never resolved, because the pooled 3-sector data used to measure it was itself contaminated (the backtest was applying the semiconductor-only seasonality/macro rules to bank and healthcare bars too, diluting the read). Fixing that contamination first, then re-measuring on clean semiconductor-only data, confirmed a real and large inversion — "seasonally strong" months won 38.9% vs. "seasonally weak" months at 70.7%. Flipped the sign; re-running the full backtest on the same historical set the sign-flip alone moved win rate from 62.3% to 66.9% and Sharpe from 3.01 to 4.16. Also found: the weight-calibration step added in v2.2.42 has been live for over a week but is a mathematical no-op — it redistributes a fixed point pool by fraction, which always sums back to the same total no matter what the calibrated weights say, so it's never once influenced a real score. And the go-live gate's "15 closed trades is enough to have an opinion" floor can't actually be passed by a model performing exactly to the current backtest's own spec — simulated it directly; a genuinely good model reads as "insufficient/failing" about 71% of the time at n=15.
...
**In short:** Reviewing that day's open paper trades surfaced three separate bugs. Biggest one: AVGO and ABBV's 2026-08-14 signals were breakout entries — buy orders that only fill if the stock actually trades up into a trigger zone — but neither stock ever got close to its zone that day. `paper_updater.py` didn't check for that; it would have booked both as stopped-out losses on the very next run, even though no real position was ever opened. Second: `shared/utils/risk_reward.py`'s stop/target math has always supported anchoring a bullish stop to a real support level and a target to the next real low-volume price area — but no live or backtest call site ever actually passed those values in, so every single trade in the paper ledger landed on exactly the mechanical 3.00 R:R fallback, never a real one. Third, smaller: the `dominant_news_theme` column has been blank on every trade since it was added, because the code was reading it back out under the wrong dictionary key.
