...
│   ├── position_rescoring.py          # Re-scores open positions daily; flags early exit when confidence drops significantly post-entry, time-stop, trailing stop
...
| **Phase 8** | Position rescoring + portfolio management | `position_rescoring.py` (daily re-scoring, all management stops), `portfolio_manager.py` (position tracking, circuit breakers, PDT tracking, entry confirmation handling, correlation override) |
