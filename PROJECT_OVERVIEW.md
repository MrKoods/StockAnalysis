...
- **Regime coverage isn't a data problem — it's structural.** `trending_down` produces zero *bullish* candidates by construction (the entry filter requires `trend_intact`); `high_vol` candidates get capped at confidence ≤70 by a deliberate safety brake and can never qualify. This design can only ever surface *bullish* signals in `trending_up`. The project's "80% WR in all four regimes independently" requirement is unsatisfiable as written for the bullish path — not something more data fixes. **Superseded for direction coverage as of v2.2.58/v2.2.59 (2026-08-18/19, this section otherwise still reflects 2026-07-19 and has not been fully refreshed since):** a real, symmetric bearish/breakdown detection path now exists end-to-end (technical, sentiment, news, positioning, regime, sector-rotation all mirror-aware) and is live for paper trading only (`enable_bearish_signals: true`) — so the model is no longer bullish-only by construction. It backtests to a negative Sharpe in every sector on data available so far (best pooled Sharpe -1.73 against the +1.0 go-live requirement), which is a real performance finding, not the old structural gap described above. See `CHANGELOG.md` v2.2.58/v2.2.59 for the full detail.
...
- **The backtest doesn't model `cross_ticker` or `earnings` at all** (both still hardcoded to `0.0` in `backtesting/simulation.py`) — so the v2.2.3 modifier-stacking fix (§11) can't be validated by backtest replay for `cross_ticker`, only by direct log inspection, and any future cross_ticker/earnings tuning will need the same treatment. `sector_rotation` no longer has this gap: v2.2.47 wired real historical SMH-vs-SPY data into the backtest, and the resulting calibration run found the `+5` inflow boost wasn't supported by real outcomes (dropped to `0`; `outflow_penalty` stayed at `-15`, which the data didn't contradict).
