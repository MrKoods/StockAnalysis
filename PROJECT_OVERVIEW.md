...
*Last reviewed: 2026-07-19, model v2.2.4 — includes the first real backtest result, a fix to correlated-modifier stacking, a fix to a previously-broken sensitivity-analysis tool, and the walk-forward result that tool fix led to examining for the first time.*
...
| **Current model version** | v2.2.4 (see `CHANGELOG.md`) |
...
- **Regime coverage isn't a data problem — it's structural, and more history won't fix it.** Pulled the full 249-candidate signal set (not just the 107 that qualified) and broke it down by regime: `trending_up`=170 (avg confidence 91.4), `high_vol`=77 (avg confidence 63.0 — **every single one** blocked by a deliberate `high_vol_score_cap: 70` safety brake), `choppy`=2, `trending_down`=0. `trending_down` produces zero candidates *by construction* — the entry filter requires `trend_intact`, which a downtrend can't satisfy. This strategy shape structurally can only ever qualify signals in `trending_up`; the project's own "80% win rate in all four regimes independently" requirement (Project_Scope.md → Performance Thresholds) is unsatisfiable as written for this design, not a gap that a longer historical window closes.
- **The sensitivity-analysis tool (`--sensitivity`) was completely broken and has been fixed (v2.2.4).** It never worked — a config-key mismatch (`metrics.py` reading a dict key nothing ever populated) meant it silently returned all zeros at every threshold since it was written. Real data now available: win rate is **roughly flat, 56.8%–60.5%, across every threshold from 85 to 95** — it does not climb as the bar tightens. That means raising the live threshold above 90 is unlikely by itself to close the gap to 80%; the confidence score isn't ranking candidates by real forward edge in this range.
- **The walk-forward validation the project already computes on every backtest run — and had never actually looked at — has never passed. Not once, in any of 24 six-month windows from 2014 to 2026.** (Pass bar: win rate ≥70%, avg R:R ≥1.8, ≥10 trades per window.) Most windows have far too few qualifying trades (many are 0-5) to be individually meaningful, and win rate swings wildly window to window (0% to 76.9%). The 57.0%/107-trade headline number is the most statistically solid figure available, but the walk-forward result shows real instability underneath it that the single fixed test-set number hides.
- **A volume-confirmation entry filter was tested and deliberately reverted, not adopted.** Requiring `breakout_volume_zscore ≥ 0.5` on the entry filter improved the fixed test-set result (57.0%→62.0% WR, R:R 2.01→2.07, drawdown 7.5%→5.7%) but dropped qualifying trades to 79 (below the 100-trade minimum) — and more importantly, tuning filter parameters by repeatedly re-running against the same fixed 30% holdout is exactly the overfitting the 70/30 split exists to prevent. Reverted. Any real version of this idea needs to be validated against the walk-forward windows, not the single test slice.
- **A separate problem, found while investigating why paper trading had produced zero signals: `regime`, `sector_rotation`, and `cross_ticker` were stacking to a uniform -24 penalty across the whole watchlist**, all three substantially derived from the same underlying SMH sector trend — one real market condition counted three times. A config/code key mismatch in `cross_ticker_analysis.py` also meant the configured `-10` sector-wide value was silently ignored in favor of a hardcoded `-5.0` the whole time. **Fixed in v2.2.3**: the key mismatch is corrected, and `sector_wide_discount` is now deliberately set to `0`. Not modeled in the backtest at all (`cross_ticker_modifier` is hardcoded to `0.0` there), so this fix doesn't change the backtest result — it only affects live/paper scoring. Full detail in `CHANGELOG.md` under v2.2.3.
- **On the economics, for context:** 57.0% win rate × 2.01 avg R:R on winners works out to roughly **+0.72R expected value per trade** — directionally a positive-EV system, just short of the project's own deliberately strict 80%/1:3 double-bar. Worth a separate conversation about whether that bar is the right one, independent of the walk-forward instability finding above.
- **Every version from v2.0.0 through v2.2.4 remains formally "not eligible to go live"** per the project's own CHANGELOG rule.
- **v2.2.2 (2026-07-19)** was a large hardening pass: 24 issues found via a full-codebase senior-engineer-style review were fixed, spanning scoring correctness, risk/execution enforcement, calibration, dead code removal, and reliability. Full detail in `CHANGELOG.md`.
- **Next concrete action:** the walk-forward result is the single most important open question — before tuning any filter or threshold further, decide whether the strategy needs a fundamentally different signal (this breakout-only design may just not have enough usable regime to work with) or whether the sample-size problem (too few trades per window) means more/different historical data is needed before any conclusion is trustworthy. Meanwhile watch daily paper trading for whether the modifier fix changes anything — base category scores (roughly 50-57/100 before modifiers, per the 2026-07-17 log) are still well short of 90 on their own, so don't expect a signal from that fix alone.
...
- **Backtest fails, and walk-forward has never passed in any of 24 windows tested (2014-2026).** 57.0% win rate on the fixed test set vs. 80% required; regime coverage outside `trending_up` is structurally unreachable for this entry-filter design, not a data gap. See §11 and `CHANGELOG.md` v2.2.2/v2.2.4. This is the single most important open item — more important than any single threshold or filter tweak.
...
1. **Decide what to do about the walk-forward result.** 0 of 24 six-month windows (2014-2026) have ever passed, and most have too few trades to judge individually. This is a strategic call, not a code fix: either the entry signal itself needs a fundamental rework (this breakout-only design may not have enough usable edge), or a longer/different historical dataset is needed before any single-window conclusion — including the 57%/107-trade headline number — can be trusted on its own.
2. **Decide on the regime-coverage requirement.** Given it's structurally unreachable for a breakout-style entry filter (§11), either redefine it (validate `trending_up` directly, validate abstention elsewhere) or treat it as a signal that this design needs a second, different signal type for non-trending regimes.
3. **If pursuing entry-filter improvements (e.g. volume confirmation), validate against the walk-forward windows, not the single fixed test set** — the fastest way to fool yourself here is tuning a filter until it improves the one 30% holdout everyone keeps re-running against.
4. Let paper trading keep accumulating real signals in parallel — no meaningful forward-test conclusion is possible yet at 0 logged trades, independent of the backtest work above.
...
7. Continue treating every scoring/threshold change as a version bump with a required backtest entry — no exceptions, per the project's own rule.
