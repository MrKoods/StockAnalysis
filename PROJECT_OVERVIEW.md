...
*Last reviewed: 2026-07-19, model v2.2.9 — includes the first real backtest result, several backtest-methodology fixes and self-corrections, a rate-regime investigation, the infrastructure to actually run a second sector live (with live activation deliberately deferred to its own future version), and a same-day correction to that infrastructure — a claimed fix was incomplete, caught when directly asked whether the second sector really gets the full scoring system. See §11.*
...
| **Current model version** | v2.2.9 (see `CHANGELOG.md`) |
...
- **Every version from v2.0.0 through v2.2.9 remains formally "not eligible to go live"** per the project's own CHANGELOG rule — the fixed-slice trade count (18) is disqualifying on its own, independent of the improved 66.7% win rate.
...
1. **Decide when/whether to flip `regional_banks.active: true`.** The infrastructure is built and tested (v2.2.8/v2.2.9) — this is now a live-trading-universe judgment call, not a technical blocker. Given v2.2.9 found a real gap in v2.2.8's own claimed fix, a careful review of the full multi-sector code path (not just re-trusting the CHANGELOG text) is warranted before activation, not just flipping the flag. Activating it requires its own real backtest against the live multi-sector code path and a MAJOR version bump per the project's own rule.
